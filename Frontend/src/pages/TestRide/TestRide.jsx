...
const TestRide = () => {
    ...
    const [zipcode, setZipcode] = useState('Enter zipcode here');
    const [locations, setLocations] = useState([]);
    const [locationId, setSelectedLocationId] = useState(null);
    const [selectedLocation, setSelectedLocation] = useState(null);
    ...
    const handleSubmit = async () => {
        try {
            const verificationResult = await verifyUserDetails(firstName, lastName, email, phoneNumber);
            console.log('User exists:', verificationResult);
            console.log(verificationResult);
            if (verificationResult) {
                const combinedDateTime = `${selectedTime}`;
    
                console.log(locationId, combinedDateTime, selectedModel);
                const action = 'remove'; // or 'add' depending on what you need to do
                const updateResponse = await updateTimeSlot(locationId, selectedModel, combinedDateTime, action);
                ...
                console.log(verificationResult.customId, locationId, selectedModel, combinedDateTime, phoneNumber);
                const demoBookingCreated = await createDemoBooking(
                    verificationResult.userId,
                    locationId,
                    selectedModel,
